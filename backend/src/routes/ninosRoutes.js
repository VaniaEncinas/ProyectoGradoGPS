...
// 📌 Obtener todos los niños de un usuario (con nombre de pulsera)
  ...
  try {
    ...
    const formattedRows = rows.map(nino => ({
      ...
      fecha_nacimiento: nino.fecha_nacimiento
        ...
        : null,
      pulsera: nino.id_pulsera ? { id_pulsera: nino.id_pulsera, nombre: nino.pulsera_nombre } : null
    }));
...
// 📌 Actualizar un niño (PATCH)
router.patch("/update/:id_nino", async (req, res) => {
  try {
...
// 📌 Eliminar un niño
router.delete("/delete/:id_nino", async (req, res) => {
  try {
    const { id_nino } = req.params;

    // Liberar la pulsera asignada al niño
